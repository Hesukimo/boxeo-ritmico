...
public class GameManager : MonoBehaviour
{
    ...
    private int thresholdSpeedup = 1000; //Cada cu�ntos puntos aumentar la velocidad
    [SerializeField] static public float SpeedMultiplier = 1.25f; //Por cu�nto aumentar la velocidad
	...
	void Start()
    {
        ...
        // Timear inicio de la canci�n un pel�n m�s tarde
        dspStartTime = AudioSettings.dspTime + 1f;
        musicSource.loop = true; // Poner canci�n en bucle
	...
	void Update()
    {
        ...
        //Reiniciar partida sin quieres pulsar el bot�n
        if (gameOverActivo)
        {
	...
	//Funci�n que comprueba lo cerca que se est� de un beat y lo devuelve de 0 a 1, 0 = lo m�s alejado del beat posible (contratiempo) y 1 exacto
	public double ComprobarRitmo()
    {
        double recentTime = songTime - lastBeatTime; //Tiempo que ha pasado desde el �ltimo beat
    ...
    void On4Beat()
    {
		...
		//A�adir aqu� c�digo que corre cada 4 beats (un comp�s entero)
	}

    //Funci�n para definir el tipo del enemigo al aparecer
    private void Tipo()
    {
